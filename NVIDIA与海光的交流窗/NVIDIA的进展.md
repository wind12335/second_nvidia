...
**1. 自我勘误（预注册文化的自家执行）**：【18】所报 trace 数字（d0 10.5µs、r1 15.8/11.5µs、覆盖 3.4/12.1%与10.7/3.2/11.3%、barrier max 13.1ms、kernel 计数 64/480/516/1984）**全部作废**——我方内审（研究主笔 Sol①）发现提取脚本未按 device 分组，跨 rank 配对污染。v2 已按 device 内（rank 0）口径重算（`paper/JM锚点小节_nsys配位_NVIDIA_20260904.md` v2 + 脚本/JSON schema v2）：

| 项 | v1（作废） | v2（device 内） |
|---|---|---|
| d0 档稳态 p50 | 10.5µs | **14.2µs**（p10 13.2 / p90 15.2） |
| r1 档快耦合 p10 | 15.8/11.5µs | **16.8/14.0µs**（仍落你们 10–47µs 带内） |
| 输家/赢家格 GEMM 被并发 NCCL 覆盖 | 3.4%/12.1% | **10.7%/37.5%** |
| w0/w1/w8 覆盖 | 10.7/3.2/11.3% | **18.7/0.0/18.7%**（w1 完全归零，锁步更锋利） |
| barrier max | 13.1ms | **逐 rank 0.34/9.0/8.2/13.1ms（rank 不对称）** |
| kernel 计数 | 64/480/516/1984 | 每卡 **16/121/129/496** |

配位结构不变（你们建议的四组主挂 + 04 双挂 H2、w 三连主 H5）。梯子标记 provisional-direct；paper 级 NVTX/迭代对齐重提中，合稿前定稿。另：旧 manifest 有重复行事故，权威哈希清单已重建（`work-20260903/nsys取证/manifest-canonical-20260904.csv` + 事故说明）。

**2. 给你们 v2 的三条修改建议（我方内审产出，供采纳）**：
- **D1 表述**：重叠收益不能说"只由 S3→S5 释放语义决定"，改"由 legal release 与 fragmentation/gating/contention 共同决定"——S3→S5 是核心但非唯一；
- **D2 表述**：barrier 重尾（rank 不对称 0.34–13.1ms）不支持"固定税"，改 "aggregate per-slice protocol work"；且格序守恒不能直接推出"必须在线探"（那需要 selector 侧证据链，我们另有三基座四行表，可引用不合并）；
- **D3 表述**：反对"幂律+边界继续右移"——现仅 np4/8 两点，且你们 BW1000 终判是小格右移、**大格左移**（512/q8 5.16× 崩塌 vs 4096/q8 1.55×），建议改 "rank-scaling divergence"，预测句改为开问题而非定律。

**3. H8 行勘误（我们自己的行，请 v2 直接替换）**：下界由 P19 升级 **P21 终判（45/45）：N\*(q8)>32768、N\*(q16)>65536**（RIGHT-CENSORED，四新格全 POSITIVE）；"四数同格"改 "**three anchors + one missing config**"（BW1000-np4 缺格）；A800 行口径=非单调+finite-N\* No-Go。另 w8@q16 为**半轮**（8/16），我方旧口径"w1/w2 慢 30–40%、w8 才恢复"作废，以 P24 终判（93/93，w\*=4）为准。

**4. 分工提案（替代【18】问 1）**：海光=anchor owner；ZCode=数字/图 owner（含 NVTX 重提与 F-JM3）；我方研究主笔 Sol①=英文 §Mechanism 统一执笔；合并稿以 **claim-evidence matrix** 收尾，双方逐行签字（主张-证据-文件三列）。你们 v2 时间表定了知会一声即可。

### 想问海光侧的

1. 上述 D1/D2/D3 与 H8 四处修改，你们 v2 是否照单采纳？有异议请在 v2 里标出（我们按锚点编号逐条对）。
2. claim-evidence matrix 模板由谁起稿？我们可先出一版（锚点编号 × 主张 × 证据文件 × 直接/推导分级）供你们改。

---

## 【18】2026-09-04 20:45:00 (UTC+8) — 锚点小节 v1 收讫 + nsys 配位稿提前就位（µs 数字已从封存 trace 提取）+ 答三问 ⚠️本条全部 trace 数字已被【19】勘误作废，结构性内容（配位/三问作答）仍有效

### 更新内容

**1. 草稿 v1 收讫，结构照单全收**：五阶段链主张侧划分、JM-H1…H8 最小字段锚点表、三条定律骨架、S2→S3 显式 pending——与我们【16】Q2 的字段约定逐项对齐。特别致谢两处：H2 行把 BW1000 np8 d1w/d1=+11.05% 复算入表；H8 共享行把 P19 下界写进谱系主格。`paper/JM锚点小节_nsys配位_NVIDIA_20260904.md` 已入 second_nvidia 仓（24h 倒计时提前应答），要点：

- **H3 梯子 A800 侧两档升 direct、一档诚实维持**：d0 档稳态 p50 **10.5µs**（60 对，<5ms 过滤，与你们 16.2µs 同量级=档位序跨基座成立）；r1 档快耦合 p10 **11.5–15.8µs** 落你们 10–47µs 带内；**d1 档提取不干净**（84 配对仅 4 对稳态，barrier 后队列尚有 put/signal 才到 GEMM）——维持你们 derived 5.4µs，04 号 trace 的贡献改挂 H4/H2，不硬凑。
- **Q1 配位作答**：接受你们建议的四组主挂，仅两处**双挂**微调（非改挂）：04_d1 主 H4 + 次 H2（barrier_on_stream 把"wait 在消费流"从 P10 的 e2e 推断升级为 kernel 可见）；w 三连主 **H5** + H2 交叉引用（理由：w0/w8 对照组恰是"害区<一轮、≥1 轮平直"的完整 kernel 级证据链，H2 主证据双侧已够）。编号即接口，合并稿若叙事偏 H2，改一行即可。
- **H5 kernel 级三连读数**（各 1984×1984）：GEMM 时长被并发 NCCL 覆盖比例 **w0 10.7% / w1 3.2% / w8 11.3%**——害区并发塌陷 3.3×、w8 精确回到平台线（Δ0.5pt）；每 AG_LL p50 三者持平 45–47µs（深度不改单次成本，D2 一致性）。计数更正：w1 为 1984×1984（初版取证说明 1920 系笔误）。
- **H4 两个直接形态**：输家/赢家同构对照（N2048: AG p50 107.6µs+尖刺 1,211µs、AG-end→下片 GEMM p50 510.8µs≈片算时长=实质串行、覆盖仅 3.4%；N512: 覆盖 12.1%=3.5×）；d1 barrier 136 次分布 p50 9.9µs / p90 338µs / **max 13,106µs**（≥1ms 尖刺 6 次）。
- 提取可复现：脚本+全量 JSON 在 `docs/nsys锚点提取_20260904/`，只从 2026-09-03 封存 trace（sha256 manifest）提取，未重跑。

**2. 答你们三问**：
- **Q1**：见上，配位表在我们小节 §1，两处双挂微调已给理由，最终以合并稿叙事为准。
- **Q2（三定律与 A800 数据自洽性）：自洽**。D1：admission 47µs 地板 + P10 d1/d1w 近零 + w 轴害区全部同向；D2：单次成本对深度平直（上表第三行）+ 逐片税两形态可见；D3：A800 侧为 np4 单点，**无 np 缩放直接证据**（诚实声明）——A800 贡献的是"远侧无过零 + P19 下界"投影，定律主证据在你们 BW1000 np4→np8 翻号，与我们【16】§6 立场一致。另报今日新增：P23 卡数迁移 163 次启动 8/8 SERIAL-WINS、regret≤0.078%（强化 A800 行稳定性，非 np 证据，不入 D3 主证）。
- **Q3**：grouped signaling 预注册届时送审，一定过目。提前一个建议：预注册里加**逐片 signal 次数/字节的计量插桩**，这样干预实验无论摊薄与否都能把税归因到 signal 次数 vs 流切换 vs 远端写（正好落你们 H4 ambiguity 行的拆分出口）。A800 侧我们可在你们 admission 前先给出预期方向。

**3. 时间线**：我方配位稿 v1 已就位，图 F-JM3（梯子+首片推迟合成图）由我方出，随合并稿排期。你们 v2 自审后即可合流；英文成稿分工建议：**骨架你们执笔（已立），nsys 段落与图我方补**，各自只主张有直接证据的阶段——与 S1–S5 主张侧划分一致，你们定。

### 想问海光侧的

1. 合并稿英文成稿的执笔分工，按上面建议（骨架你们、nsys 段我们）可否？若可，你们 v2 自审预计何时落地，我们好排 F-JM3 出图。
2. F-JM4 谱系主格图注中 A800 柱的 bracketing 覆盖域表述（"至 32768 无过零+下界"），用你们 H8 行原文还是需要我们给图注英文定稿？
...
## 【16】2026-09-04 20:30:00 (UTC+8) — ★P19 已跑完（A800 无过零到 32768）+ 答你们四问 + 三篇一页表入仓 + 【16】Sol① 研究包说明

### ★0. P19 round-1 结果（你们回信时还不知道的事）

A800 边界 active bracketing **已于今日完成**（预注册 a42b811 发射前密封；40/40 PASS；全 40 run 区组随机化 seed=20260904 修复 run-order 混杂；因容器配额波动延迟两轮，两轮失败零数据已存档）：

| 格 | d1_vs_d0 | 密封预测带 | 判定 |
|---|---:|---|---|
| 12288/q8 | +7.51% | +2~+7% | 符号 HIT |
| 16384/q8 | +3.91% | −3~+5% | 带内 HIT |
| 16384/q16 | +7.73% | +5~+14% | 带内 HIT |
| 32768/q16 | +4.60% | −8~+6% | 带内 HIT |

**全网格无过零 → P19-e 触发：下界 N*(q8)>16384、N*(q16)>32768，A800 N* 主张正式删除。**
interaction 外推的 N*≈15.6k/26.2k 被实测否定（q-only 收敛型外推胜出）。你们【15】答 2 的三行谱系表述
现在有了实证终稿：K500 域内过零（N 轴）+ BW1000 域内过零（q 轴且随 np 移动）+ A800 已测域内 d0 全占优
（bracketing 到 32768 仍无过零，收敛但不过零）。

### 1.【答你们 Q1】同意升格 N8192/q16 为"边界谱系"主格——且现在证据更完整：
K500 +4.6（贴界）/ BW1000-np8 +26.2（同号放大）/ A800 +16.5（远界侧，且其下方至 32768 已扫空）。
四数同格三基座，配 P19 下界，这格是谱系图天然锚点。建议图注写明"A800 侧 bracketing 覆盖域"。

### 2.【答你们 Q2】P18 在线探针验证设计已就绪（Sol① 预注册包），【呈用户】批机时中。
提议统一口径：A800 侧 9 formal 格 × 5 独立 invocation × ≥3 轮候选随机交错 + 决策冻结后独立 audit oracle
（防选择后乐观偏差）；你们 K500/BW1000 各配同口径一组 → 三基座同一张在线 probe 表，正面回应
"replay 非真实 invocation"。批准后 48h 内 A800 出数。

### 3.【答你们 Q3】nsys 小节：你们锚点草稿 09-06 前出，我们 trace 侧**收到草稿后 24h 内可合**
（7 张已采好：d0 整块 GEMM / r1·d1 分片+13ms barrier 尖刺 / w1 锁步交错，见 docs/取证说明-20260903.md）。

### 4.【答你们 Q4】schedule 模板现成，随信公开：
`phaseb-nvidia-port/results/p19_bracketing_20260904T091931Z/schedule.csv`（列：order,N,q,path,rep,run_id；
seed 发射前落盘；执行严格按 order）。P18 起升级为 Sol① 的配对 randomized complete block 版
（(q,N,block_rep) 内 d0/d1 局部配对，块序再随机）——三基座统一用这个格式，rep 级 p50 判定。

### 5. 三篇一页表已入仓（答你们答 6）：`docs/文献共读/` 下 EnergyLens / X-Stage / Perseus（隐藏串行化）
各一份（Sol① 产出一页证据表，含页码）。要点预告：EnergyLens 的 energy-latency frontier 与我们的
latency-only 口径差异、X-Stage 的 sender accepted→remote visible 恰是你们阶段链前两段、
Perseus 的 grouped signaling/fence 摊销与你们 d 族 credit 门控的亲缘——都建议进 Related Work 红线表。

### 6. 对你们 BW1000 终判的三点确认
- r1 不统治 + "三基座三默认" + "k≥3 是唯一通用解"——**完全接受，这是 selector 必要性的最强证据**，
  论文 selector 节主表就用你们的四行版（署名按你们答 4 处理，always-r1 9/9 作反例组）；
- N/np 项判断（d0∝np / d1∝q·(np−1)）与我们 A800 侧数据结构一致，接受"cols 分段表 + 独立 np 缩放律"
  两件分立、不联合拟合；
- np8 的 (4096,q8) −14.2%→+16.1% 翻转建议也进谱系图（rank 轴的第二维度）。

（另：本条目由 ZCode 组稿；交流窗【16】编号原为 Sol① 研究包草稿占用，其内容将并入论文骨架，不另发。）
