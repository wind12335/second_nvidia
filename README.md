...
1. **写完即入库**：新文档、新实验脚本、新发现（含负结果），完成后立即 commit + push 到本仓库，不留"只在机器上"的状态。
2. **日期标注**：文件名一律带 `-YYYYMMDD` 后缀；新的一天产出的文件必须用当天日期，不复用旧文件名；文档内容首行标日期。
3. **工作目录**：每日工作根目录 `seconde-paper/work-<今日日期>/`（结论与报告落这里），本仓库同步镜像其中可版本化的内容。
4. **里程碑提交**：每个实验阶段（矩阵完成/trace/准入/formal…）至少一个 commit，message 写明里程碑与关键数字。
5. **原始大文件**：逐迭代 CSV、trace、日志不入 git，走时间戳 tar 包 + SHA256（见"数据纪律"），git 内保留清单与合并分析表。
6. **交流窗**：`NVIDIA与海光的交流窗/NVIDIA的进展.md`——每个里程碑更新（秒级时间戳+内容+给海光侧的问题），海光侧读此文件了解本侧动向；对等文件 `海光的进展.md` 由对方维护。
