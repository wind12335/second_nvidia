...
__global__ void dev_put_signal(uint32_t* dst, uint32_t* src, size_t bytes, uint64_t* sig, int pe) {
  if (threadIdx.x == 0 && blockIdx.x == 0)
    nvshmem_putmem_signal(dst, src, bytes, sig, 42, NVSHMEM_SIGNAL_SET, pe);
}
...
int main(int argc, char** argv) {
  ...
  if (getenv("EXPLICIT_P2P")) {
    int can=0, peer_dev = (pe==0)?1:0;
    CK(cudaDeviceCanAccessPeer(&can, nvshmem_team_my_pe(NVSHMEMX_TEAM_NODE), peer_dev));
    cudaError_t e = cudaDeviceEnablePeerAccess(peer_dev, 0);
    printf("pe%d canAccessPeer=%d enablePeerAccess=%s(%s)\n", pe, can,
           e==cudaSuccess?"OK":(e==cudaErrorPeerAccessAlreadyEnabled?"ALREADY":"ERR"), cudaGetErrorName(e));
    fflush(stdout);
  }
  if (pe == 0) {
    ...
    if (strcmp(mode,"raw_memcmp")==0) {
      // 裸 CUDA 判别：同型 memcpyAsync(D2D,peer) vs cudaMemcpyPeerAsync
      cudaError_t e1 = cudaMemcpyAsync(rbuf, buf, bytes, cudaMemcpyDeviceToDevice, s);
      printf("pe0 raw cudaMemcpyAsync(D2D,peer) -> %s\n", cudaGetErrorName(e1)); fflush(stdout);
      cudaError_t e2 = cudaMemcpyPeerAsync(rbuf, 1, buf, 0, bytes, s);
      printf("pe0 raw cudaMemcpyPeerAsync -> %s\n", cudaGetErrorName(e2)); fflush(stdout);
      CK(cudaStreamSynchronize(s));
      uint32_t host_w=0; CK(cudaMemcpy(&host_w, rbuf, 4, cudaMemcpyDeviceToHost));
      printf("pe0 readback peer first word %08X expect A5A5A5A5 -> %s\n", host_w, host_w==0xA5A5A5A5u?"PASS":"FAIL"); fflush(stdout);
    } else if (strcmp(mode,"device_put")==0) {
      // 设备端 API：launcher kernel 内调 nvshmem_putmem_signal（device transport 路径）
      dev_put_signal<<<1,32,0,s>>>(rbuf, buf, bytes, rsig, 1);
      CK(cudaGetLastError());
